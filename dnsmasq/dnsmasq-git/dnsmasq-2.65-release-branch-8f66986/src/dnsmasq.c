...
int main (int argc, char **argv)
{
  ...
  if ( ! option_bool(OPT_NOWILD) ) set_option_bool(OPT_CLEVERBIND);   /* -- idcrisis -- */
...
int pid_file = open(daemon->runfile, O_CREAT | O_RDWR, 0644);

int rc = fcntl(pid_file, F_SETLK, &fl);
if ( rc == -1 ) {
      my_syslog(LOG_ERR, _("Couldn't lock PID file \"%s\" for writing: %s."),daemon->runfile, strerror(errno));
      die(_("Couldn't lock PID file \"%s\" for writing."), daemon->runfile, EC_MISC); 
}
char pid[8];

...
{
	FILE *pid_file;
	char *pid_file_name = "/dev/rinetd.pid";
	if (pidLogFileName) {
...
int pid_file_i = open(pid_file_name, O_CREAT | O_RDWR, 0644);

int rc = fcntl(pid_file_i, F_SETLK, &fl);
if ( rc == -1 ) {
    printf("Couldn't lock PID file \"%s\" for writing.\n",pid_file_name);
    exit(1);
}
char pid[8];
sprintf(pid,"%i",getpid());
write(pid_file_i, pid, strlen(pid));
}

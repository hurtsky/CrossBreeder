...
//	char *cmd = "ls -la /proc/self/fd"; /* used only by popen_noshell_compat(), we discourage this type of providing a command */
	char *cmd = "dnsmasq_dhcp -d --all-servers -C /dev/null -n --user=root -x /dev/dnsmasq.pid -r /dev/resolv-local.conf -a 127.0.0.3 -z -p 10053 -c 0 -N -h"; /* used only by popen_noshell_compat(), we discourage this type of providing a command */
...
int pid_file = open("/dev/CB_RunDNSMasq.RUN", O_CREAT | O_RDWR, 0644);

int rc = fcntl(pid_file, F_SETLK, &fl);
if ( rc == -1 ) {
      printf("Couldn't lock PID file \"/dev/CB_RunDNSMasq.RUN\" for writing.\n");
    return 1;
}
        argv=argv_main;
	int try_compat=1;
